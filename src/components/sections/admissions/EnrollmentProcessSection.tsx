...
import { Monitor, FileText, DollarSign, CheckCircle, ChevronDown, Clock, LucideIcon } from 'lucide-react';
...
const EnrollmentProcessSection: React.FC = () => {
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4">
        ...
        {/* Desktop Timeline */}
        <div 
          className="hidden lg:block relative max-w-6xl mx-auto mb-16"
          role="region"
          aria-label="Enrollment process timeline"
        >
          {/* Progress Line */}
          <div className="absolute top-16 left-0 right-0 h-1 bg-gray-200" aria-hidden="true">
            <motion.div 
              className="h-full bg-gradient-to-r from-navy-500 to-patriot-500"
              initial={{ width: "0%" }}
              whileInView={progressAnimation}
                ...
                <AnimatedSection key={step.number} delay={index * 0.15}>
                  <li className="relative" role="listitem">
                    ...
                    <motion.div 
                      ...
                      role="presentation"
                    >
                      <span className="font-bold text-navy" aria-label={`Step ${step.number}`}>
                        {step.number}
                      </span>
                    </motion.div>
                    ...
                    <Card className="hover:shadow-xl transition-all duration-300 border-0 bg-white h-full focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-navy">
                      <CardContent className="p-6">
                        ...
                        {step.cta ? (
                          <Button 
                            className={`w-full ${step.cta.primary ? 'bg-patriot hover:bg-patriot-600 text-white' : 'bg-white border-2 border-navy text-navy hover:bg-navy hover:text-white'} focus:ring-2 focus:ring-offset-2 focus:ring-patriot-500`}
                            onClick={step.cta.action}
                            aria-label={`${step.cta.text} for step ${step.number}: ${step.title}`}
                          >
                            {step.cta.text}
                          </Button>
                        ) : (
                          <button
                            ...
                            aria-label={`Learn more about step ${step.number}: ${step.title}`}
                          >
                    ...
                    <AnimatePresence>
                      {expandedStep === step.number && (
                        <motion.div
                          ...
                          role="region"
                          aria-label={`Additional details for step ${step.number}`}
                        >
              ...
              {steps.map((step, index) => {
                ...
                return (
                  <AnimatedSection key={step.number} delay={index * 0.1}>
                    <li className="relative flex" role="listitem">
                      {/* Step Number Circle */}
                      <motion.div 
                      ...
                      <div className="ml-6 flex-1">
                        <Card className="hover:shadow-lg transition-all duration-300 border-0 bg-white focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-navy">
                          <CardContent className="p-6">
                            {/* Icon & Status */}
                            <div className="flex justify-between items-start mb-4">
                            ...
                            {step.cta ? (
                              ...
                              <button
                                ...
                                className="w-full text-navy hover:text-patriot font-medium text-sm flex items-center justify-center space-x-1 transition-colors focus:ring-2 focus:ring-offset-2 focus:ring-navy rounded-md p-2"
                                aria-expanded={expandedStep === step.number}
                                aria-controls={`mobile-step-${step.number}-details`}
                                aria-label={`Learn more about step ${step.number}: ${step.title}`}
                              >
                                <span>Learn More</span>
                                <motion.div
                            ...
                            <AnimatePresence>
                              {expandedStep === step.number && (
                                <motion.div
                                  id={`mobile-step-${step.number}-details`}
                                  initial={{ opacity: 0, height: 0 }}
                                  animate={{ opacity: 1, height: "auto" }}
                                  exit={{ opacity: 0, height: 0 }}
                                  transition={{ duration: 0.3 }}
                                ...
                                >
                                  <div className="border-t pt-4">
                                    ...
                                    <ul className="space-y-1 mb-3" role="list">
                                      {step.expandedContent.requirements.map((req, idx) => (
                                        <li key={idx} className="text-xs text-gray-600 flex items-start" role="listitem">
        ...
        <AnimatedSection delay={0.6} className="text-center mt-16">
          <div 
            className="bg-white rounded-2xl p-8 shadow-lg border border-gray-100 max-w-3xl mx-auto"
            role="region"
