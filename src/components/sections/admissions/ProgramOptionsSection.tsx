...
import { Plane, Building, ChevronDown, Check, Calendar } from 'lucide-react';
    ...
    <section className="py-20 bg-white">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-6xl mx-auto mb-12">
          {programs.map((program, index) => (
            <AnimatedSection key={index} delay={index * 0.2}>
              <Card className="h-full group hover:shadow-2xl transition-all duration-500 border-2 border-gray-100 bg-white overflow-hidden">
                <CardContent className="p-8">
                  ...
                  <div className="flex justify-between items-start mb-6">
                    ...
                    <div className="bg-patriot text-white px-4 py-2 rounded-lg font-semibold text-lg">
                      {program.price}
                    </div>
                  </div>
                  ...
                  <Link href={program.link}>
                    <Button 
                      variant="outline" 
                      className="w-full border-2 border-navy text-navy hover:bg-navy hover:text-white font-semibold py-6 text-lg transition-all duration-300"
                    >
                      {program.ctaText}
                    </Button>
                  </Link>
        ...
        <AnimatedSection delay={0.4}>
          <div className="max-w-4xl mx-auto">
            <motion.button
              ...
              className="mx-auto flex items-center space-x-2 text-navy hover:text-patriot transition-colors duration-300 font-semibold text-lg mb-8"
                ...
                >
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 pt-3">
                    {dayOptions.map((option, index) => (
                      ...
                      >
                        <Card className="relative h-full hover:shadow-lg transition-all duration-300 border-2 border-gray-100 overflow-visible">
                          {option.popular && (
                            <div className="absolute -top-2 right-4 bg-patriot text-white px-3 py-1 rounded-full text-xs font-semibold z-10">
                              Most Popular
                            </div>
                          )}
        ...
        <AnimatedSection delay={0.6} className="text-center mt-16">
          <div className="bg-gradient-to-r from-navy-50 to-patriot-50 rounded-2xl p-8 border border-gray-200 max-w-3xl mx-auto">
